...
class ZG_Model extends CI_Model
{
	/**
	...
	public function __construct()
	{
		...
		if (empty($this->_table))
		{
			...
			$this->_table = preg_replace('((_model)?$)', '', strtolower(get_class($this)));
			$this->_logger = $this->logger->getLogger('Model: ' . get_class($this));
		}
	}
	...
	{
		$data=(object)$data;
		$primary_key = $this->_table . '_' . $this->primary_key;
		if(!empty($data->{$primary_key})){
			return $this->update($data->{$primary_key}, $data, $skip_validation);
		}else{
	 ...
	 */
	private function _get_offset($page, $page_num){
	 ...
	 */
	public function count($query_param = array()){
		$this->db->select('COUNT(*) as count');
		if($query_param){
			$this->db->where($query_param);
		}
